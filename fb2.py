...
def main():
	...
	print("FACEBOOK SCORE\n")
	# print(facebookScore(value))

def facebookScore(value):
	totalScore=0
	try:
	    value=json.loads(val,strict=False)
	    email=value['email']
	    try:
	        ...
	        if(containsfirst.search(email)):totalScore+=15
	    except:
	        pass
	    try:
	        lastName=value['name'].split(" ")[-1]
	        containslast=re.compile(lastName.lower())
	        if(containslast.search(email)):totalScore+=15
	    except:
	        pass
	except:
	    pass
